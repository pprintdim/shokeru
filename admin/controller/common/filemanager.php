...
class ControllerCommonFileManager extends Controller {
	public function index() {
		...
		// Фільтр типу медіа: image | video | '' (усе)
		$filter_ext = (isset($this->request->get['filter_ext']) && in_array($this->request->get['filter_ext'], array('image', 'video'))) ? $this->request->get['filter_ext'] : '';
		...
		if (substr(str_replace('\\', '/', realpath($directory) . '/' . $filter_name), 0, strlen(DIR_IMAGE . 'catalog')) == str_replace('\\', '/', DIR_IMAGE . 'catalog')) {
			...
			// Get files (за типом: лише картинки / лише відео / усе)
			if ($filter_ext == 'video') {
				$glob_ext = 'mp4,webm,ogv,mov,m4v,MP4,WEBM,OGV,MOV,M4V';
			} elseif ($filter_ext == 'image') {
				$glob_ext = 'jpg,jpeg,png,gif,webp,svg,svgz,JPG,JPEG,PNG,GIF,WEBP,SVG,SVGZ';
			} else {
				$glob_ext = 'jpg,jpeg,png,gif,webp,svg,svgz,mp4,webm,ogv,mov,m4v,JPG,JPEG,PNG,GIF,WEBP,SVG,SVGZ,MP4,WEBM,OGV,MOV,M4V';
			}
			$files = glob($directory . '/' . $filter_name . '*.{' . $glob_ext . '}', GLOB_BRACE);
	...
	public function upload() {
		...
		// Фільтр типу при завантаженні (image/video) — не змішуємо
		$filter_ext = (isset($this->request->get['filter_ext']) && in_array($this->request->get['filter_ext'], array('image', 'video'))) ? $this->request->get['filter_ext'] : '';
		$ext_image = array('jpg', 'jpeg', 'gif', 'png', 'webp', 'svg', 'svgz');
		$ext_video = array('mp4', 'webm', 'ogv', 'mov', 'm4v');
		...
		if (!$json) {
			...
			foreach ($files as $file) {
				if (is_file($file['tmp_name'])) {
					...
					// Allowed file extension types (звужуємо за filter_ext)
					if ($filter_ext == 'video') {
						$allowed = $ext_video;
					} elseif ($filter_ext == 'image') {
						$allowed = $ext_image;
					} else {
						$allowed = array_merge($ext_image, $ext_video);
					}
				...
				if (!$json) {
					...
					if ($ext === 'svg') {
					    ...
					    if (preg_match('#<script|onload=|onmouseover=#i', $content)) {
					        $json['error'] = 'SVG містить заборонений код';
					    }
					}
