...
class ControllerCatalogProduct extends Controller {
	...
	protected function getForm() {
		...
		// YouTube відео товару (id ролика)
		if (isset($this->request->post['video'])) {
			$data['video'] = $this->request->post['video'];
		} elseif (!empty($this->request->get['product_id'])) {
			$vq = $this->db->query("SELECT video FROM " . DB_PREFIX . "product WHERE product_id = '" . (int)$this->request->get['product_id'] . "'");
			$data['video'] = $vq->num_rows ? $vq->row['video'] : '';
		} else {
			$data['video'] = '';
		}
