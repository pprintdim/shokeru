...
//			}
		...
		// Список полів для соцмереж
		$social_fields = [
		    'facebook'  => 'Посилання на сторінку Facebook',
		    'instagram' => 'Посилання на профіль Instagram',
		    'tiktok'    => 'Посилання або @username TikTok',
		    'youtube'   => 'Посилання на канал YouTube',
		    'telegram'  => 'Посилання або @username Telegram'
		];

		$data['social_fields'] = [];

		foreach ($social_fields as $field => $placeholder) {
		    $key = 'config_' . $field;
		...
		// Мультимовні поля config_meta_title
		...
		// Мультимовні поля config_meta_description
		...
		// Мультимовне поле config_address
		if (isset($this->request->post['config_address'])) {
			...
			foreach ($data['languages'] as $language) {
				...
				$data['config_address'][$language['language_id']] = isset($config_address[$language['language_id']]) ? $config_address[$language['language_id']] : '';
			}
		}


		// Поле config_open (мультимовне, як приклад)
		if (isset($this->request->post['config_open'])) {
		    ...
		    foreach ($data['languages'] as $language) {
		        ...
		        $data['config_open'][$language['language_id']] = isset($config_open[$language['language_id']]) ? $config_open[$language['language_id']] : '';
		    }
		}

		// Мультимовне поле config_geocode
		if (isset($this->request->post['config_geocode'])) {
		    foreach ($data['languages'] as $language) {
		        $data['config_geocode'][$language['language_id']] = $this->request->post['config_geocode'][$language['language_id']] ?? '';
		    }
		} else {
		    $config_geocode = $this->config->get('config_geocode');
		    foreach ($data['languages'] as $language) {
		        $data['config_geocode'][$language['language_id']] = isset($config_geocode[$language['language_id']]) ? $config_geocode[$language['language_id']] : '';
		    }
		}

		if (isset($this->request->post['config_map_latitude'])) {
			$data['config_map_latitude'] = $this->request->post['config_map_latitude'];
		} else {
			$data['config_map_latitude'] = $this->config->get('config_map_latitude');
		}

		if (isset($this->request->post['config_map_longitude'])) {
			$data['config_map_longitude'] = $this->request->post['config_map_longitude'];
		} else {
			$data['config_map_longitude'] = $this->config->get('config_map_longitude');
		}


		// Поле config_comment (мультимовне)
		if (isset($this->request->post['config_comment'])) {
		    foreach ($data['languages'] as $language) {
		        $data['config_comment'][$language['language_id']] = $this->request->post['config_comment'][$language['language_id']] ?? '';
		    }
		} else {
		    foreach ($data['languages'] as $language) {
		        $config_comment = $this->config->get('config_comment');
		        $data['config_comment'][$language['language_id']] = isset($config_comment[$language['language_id']]) ? $config_comment[$language['language_id']] : '';
		    }
		}
		...
		if (isset($this->request->post['config_turbo_sms_token'])) {
			$data['config_turbo_sms_token'] = $this->request->post['config_turbo_sms_token'];
		} else {
			$data['config_turbo_sms_token'] = $this->config->get('config_turbo_sms_token');
		}
		if (isset($this->request->post['config_turbo_sms_name'])) {
			$data['config_turbo_sms_name'] = $this->request->post['config_turbo_sms_name'];
		} else {
			$data['config_turbo_sms_name'] = $this->config->get('config_turbo_sms_name');
		}

		if (isset($this->request->post['config_sale_line'])) {
			foreach ($data['languages'] as $language) {
				$data['config_sale_line'][$language['language_id']] = $this->request->post['config_sale_line'][$language['language_id']];
			}
		} 
		else {
			foreach ($data['languages'] as $language) {
				$config_address = $this->config->get('config_sale_line');
				$data['config_sale_line'][$language['language_id']] = isset($config_address[$language['language_id']]) ? $config_address[$language['language_id']] : '';
			}
		}
		...
		if (isset($this->request->post['config_logo']) && is_file(DIR_IMAGE . $this->request->post['config_logo'])) {
			$data['logo'] = $this->model_tool_image->resize($this->request->post['config_logo'], 400, 80);
		} elseif ($this->config->get('config_logo') && is_file(DIR_IMAGE . $this->config->get('config_logo'))) {
			$data['logo'] = $this->model_tool_image->resize($this->config->get('config_logo'), 400, 80);
		} else {
			$data['logo'] = $this->model_tool_image->resize('no_image.png', 400, 80);
		}

		// Мобільний логотип (навігація)
		if (isset($this->request->post['config_logo_mob'])) {
			$data['config_logo_mob'] = $this->request->post['config_logo_mob'];
		} else {
			$data['config_logo_mob'] = $this->config->get('config_logo_mob');
		}

		if (isset($this->request->post['config_logo_mob']) && is_file(DIR_IMAGE . $this->request->post['config_logo_mob'])) {
			$data['logo_mob'] = $this->model_tool_image->resize($this->request->post['config_logo_mob'], 100, 100);
		} elseif ($this->config->get('config_logo_mob') && is_file(DIR_IMAGE . $this->config->get('config_logo_mob'))) {
			$data['logo_mob'] = $this->model_tool_image->resize($this->config->get('config_logo_mob'), 100, 100);
		} else {
			$data['logo_mob'] = $this->model_tool_image->resize('no_image.png', 100, 100);
		}
		...
		// Продуктивність
		foreach (array('config_webp', 'config_minify_css', 'config_minify_js') as $perf_key) {
			if (isset($this->request->post[$perf_key])) {
				$data[$perf_key] = $this->request->post[$perf_key];
			} else {
				$data[$perf_key] = $this->config->get($perf_key);
			}
		}
