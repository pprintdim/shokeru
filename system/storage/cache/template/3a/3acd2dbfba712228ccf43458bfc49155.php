...
class __TwigTemplate_641a4c99f9654debab35629366619835 extends Template
{
...
document.addEventListener(\"DOMContentLoaded\", function () {
    ...
    document.querySelectorAll(\".filters-selected-delete\").forEach(btn => {
        btn.addEventListener(\"click\", () => {
            ...
            // знімаємо галочку у всіх чекбоксах з цим значенням
            document.querySelectorAll(`input[type=\"checkbox\"][value=\"\${filterId}\"]`).forEach(cb => cb.checked = false);
            submitFilters(); // оновлюємо фільтри
        });
    });
    // Сортування
    ...
    // Основна функція, яка формує URL і перенаправляє
    function submitFilters() {
    // ✅ Беремо поточні параметри з URL як базу
    const currentParams = new URLSearchParams(window.location.search);
    const params = new URLSearchParams();

    // Зберігаємо route якщо є
    ...
    if (searchInput && searchInput.value.trim() !== '') {
        ...
        // ✅ Якщо поле порожнє — беремо з URL
        params.set('search', currentParams.get('search'));
    }

    // Додаємо фільтри (з форми — пріоритет)
    const checkedFilters = filterForm.querySelectorAll('input[name=\"filter[]\"]:checked');
    if (checkedFilters.length > 0) {
        checkedFilters.forEach(input => params.append('filter[]', input.value));
    } else {
        // ✅ Якщо нічого не вибрано у формі — беремо з URL
        currentParams.getAll('filter[]').forEach(v => params.append('filter[]', v));
    }
    ...
    const priceTo   = filterForm.querySelector('input[name=\"price-to\"]');
        ...
        else if (currentParams.has('price-from')) params.set('price-from', currentParams.get('price-from'));

        if (priceTo?.value) params.set('price-to', priceTo.value);
        else if (currentParams.has('price-to')) params.set('price-to', currentParams.get('price-to'));
        ...
        if (sortValue) {
            ...
            params.set('order', currentParams.get('order'));
        }
