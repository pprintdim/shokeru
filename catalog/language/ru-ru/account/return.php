<?php
// Heading
$_['heading_title']      = 'Возврат товара';

// Text
$_['text_account']       = 'Личный Кабинет';
$_['text_return']        = 'Информация о возврате';
$_['text_return_detail'] = 'Детали возврата';
$_['text_description']   = '<p>Пожалуйста, заполните форму запроса на возврат товара.</p>';
$_['text_order']         = 'Информация о заказе';
$_['text_product']       = 'Информация о товаре';
$_['text_reason']        = 'Информация о причине возврата';
$_['text_message']       = '<p>Вы отправили запрос на возврат.</p><p> Уведомления о статусе запроса вы получите на ваш e-mail. Спасибо!</p>';
$_['text_return_id']     = '№ Запроса на возврат';
$_['text_order_id']      = '№ Заказа';
$_['text_date_ordered']  = 'Дата заказа';
$_['text_status']        = 'Статус';
$_['text_date_added']    = 'Добавлено';
$_['text_comment']       = 'Комментар для повернення';
$_['text_history']       = 'История возвратов';
$_['text_empty']         = 'У Вас не было раньше возвратов товаров!';
$_['text_agree']         = 'Я прочитал <a href="%s" class="agree"><b>%s</b></a> и согласен с условиями ';
$_['text_product_unpacked'] = 'Товар распакован';

// Select reason
$_['text_reason_0'] = 'Испорчен по прибытии';
$_['text_reason_1'] = 'Неисправность';
$_['text_reason_2'] = 'Ошибка заказа';
$_['text_reason_3'] = 'Получен неверный товар';
$_['text_reason_4'] = 'Другое';

// Button
$_['button_remove'] = 'Удалить';
$_['button_submit'] = 'Отправить запрос';
$_['button_add'] = 'добавить товар';

// Column
$_['column_return_id']   = '№ Запроса на возврат';
$_['column_order_id']    = '№ Заказа';
$_['column_status']      = 'Статус';
$_['column_date_added']  = 'Добавлено';
$_['column_customer']    = 'Клиент';
$_['column_product']     = 'Название товара';
$_['column_model']       = 'Модель';
$_['column_quantity']    = 'Количество';
$_['column_price']       = 'Цена';
$_['column_opened']      = 'Открыто';
$_['column_comment']     = 'Дополнительная информация';
$_['column_reason']      = '* Причина возврата';
$_['column_action']      = 'Действие';
$_['column_order']       = 'Номер заказка';


// Entry
$_['entry_order_id']     = '№ Заказа';
$_['entry_date_ordered'] = 'Дата заказа';
$_['entry_firstname']    = 'Имя';
$_['entry_lastname']     = 'Фамилия';
$_['entry_thirdname']    = 'Отчество';
$_['entry_email']        = 'E-Mail';
$_['entry_telephone']    = 'Телефон';
$_['entry_product']      = 'Наименование товара';
$_['entry_model']        = 'Модель';
$_['entry_quantity']     = 'Количество';
$_['entry_reason']       = 'Причина возврата';
$_['entry_opened']       = 'Товар распакован';
$_['entry_fault_detail'] = 'Описание дефектов';
$_['entry_comment'] = 'Коментарий о возаврате';


// Error
$_['text_error']         = 'Запрос на возврат не найден!';
$_['error_order_id']     = 'Не указан № заказа!';
$_['error_firstname']    = 'Имя должно быть от 1 до 32 символов!';
$_['error_lastname']     = 'Фамилия должна быть от 1 до 32 символов!';
$_['error_email']        = 'E-Mail адрес введен неверно!';
$_['error_telephone']    = 'Номер телефона должен быть от 3 до 32 символов!';
$_['error_product']      = 'Наименование товара  должно быть от 3 до 255 символов!';
$_['error_model']        = 'Название модели должно быть от 3 до 64 символов!';
$_['error_reason']       = 'Необходимо указать причину возврата товара!';
$_['error_agree']        = 'Вы должны прочитать и согласится с %s!';


$_['heading_form']   = 'Пожалуйста, заполните форму для запроса на возврат товара';
$_['entry_captcha']  = 'Введите код с картинки';
$_['error_captcha']  = 'Неверно введён код с картинки!';
