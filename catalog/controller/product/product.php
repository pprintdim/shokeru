	...
	public function index() {
		...
		if ($product_info) {
			...
			*/
			...
			// YouTube-відео товару — прямий запит + нормалізація (id або будь-яке посилання)
			$vq = $this->db->query("SELECT video FROM " . DB_PREFIX . "product WHERE product_id = '" . (int)$product_id . "'");
			$raw_video = $vq->num_rows ? trim($vq->row['video']) : '';
			$data['video'] = '';
			if ($raw_video !== '') {
				if (preg_match('~(?:youtu\.be/|v=|embed/|shorts/)([A-Za-z0-9_-]{11})~', $raw_video, $vm)) {
					$data['video'] = $vm[1];
				} elseif (preg_match('~^[A-Za-z0-9_-]{11}$~', $raw_video)) {
					$data['video'] = $raw_video;
				}
			}
			...
			// повнорозмірне зображення (без ресайзу, лише webp) — для великого слайдера на ст. товару
			if ($product_info['image']) {
				$data['full'] = $this->model_tool_image->webp($product_info['image']);
			} else {
				$data['full'] = '';
			}
			...
			foreach ($results as $result) {
				$data['images'][] = array(
					...
					'full'  => $this->model_tool_image->webp($result['image']),
			...
			// «Подобрано Для Вас»: якщо пов'язаних мало — доповнюємо популярними
			$picked_limit = 10;

			if (count($data['products']) < $picked_limit) {
				$added_ids = array($this->request->get['product_id'] => true);
				foreach ($data['products'] as $pp) {
					$added_ids[$pp['product_id']] = true;
				}

				$popular = $this->model_catalog_product->getProducts(array(
					'sort'  => 'p.viewed',
					'order' => 'DESC',
					'start' => 0,
					'limit' => $picked_limit + 5
				));
				...
				foreach ($popular as $result) {
					...
					if (isset($added_ids[$result['product_id']])) {
						continue;
					}
					$added_ids[$result['product_id']] = true;
					...
					$image = $result['image']
						...
						: $this->model_tool_image->resize('placeholder.png', $this->config->get('theme_' . $this->config->get('config_theme') . '_image_related_width'), $this->config->get('theme_' . $this->config->get('config_theme') . '_image_related_height'));
