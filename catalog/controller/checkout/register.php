...
class ControllerCheckoutRegister extends Controller {
	...
	public function save() {
		...
		// Реєстрація нового покупця — тільки після підтвердження email кодом
		if (!$json) {
			$verified = strtolower((string)($this->session->data['checkout_email_verified'] ?? ''));

			if ($verified === '' || $verified !== strtolower(trim($post['email']))) {
		...
		if (!$json) {
			...
			// оборотне збереження пароля (показ власнику в кабінеті) + код одноразовий
			$this->storePasswordEnc($this->request->post['email'], $generated_password);
			unset($this->session->data['checkout_email_verified']);
	...
	/* ===== Реєстрація на checkout: надіслати код підтвердження на email ===== */
	public function sendCode() {
		...
		if (!$json) {
			if (!filter_var($email, FILTER_VALIDATE_EMAIL) || utf8_strlen($email) > 96) {
				$json['error'] = $this->language->get('error_email');
			} elseif (utf8_strlen($firstname) < 1) {
		...
		if (!$json) {
			...
			$this->session->data['checkout_otp'] = array(
				'email'    => $email,
				'code'     => $code,
				'expires'  => time() + 600,
				'attempts' => 0,
	...
	/* ===== Реєстрація на checkout: перевірити введений код ===== */
	public function verifyCode() {
		...
		$otp   = $this->session->data['checkout_otp'] ?? null;

		if (!$otp || strtolower($otp['email']) !== strtolower($email)) {
			$json['error'] = $this->language->get('error_code_invalid');
		} elseif (time() > $otp['expires']) {
			unset($this->session->data['checkout_otp']);
			...
			$this->session->data['checkout_otp']['attempts']++;
			$json['error'] = $this->language->get('error_code_wrong');
		} else {
			$this->session->data['checkout_email_verified'] = strtolower($email);
			unset($this->session->data['checkout_otp']);
			$json['success'] = true;
		}
	...
	/* ===== Скинути підтвердження email (натиснули «редагувати») ===== */
